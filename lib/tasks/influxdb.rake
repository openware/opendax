namespace :influxdb do
  ...
  task :create do
    sh 'docker-compose run --rm influxdb cat influxdb.sql | influx'
  end

  desc 'Drop arke_production database'
  task :drop do
    sh "docker-compose run --rm echo 'drop database arke_production' | influx"
  end
